...
# -*- coding: utf-8 -*-
"""
Start Page — home base: malote display, tipo cards, search, export
"""
...
def create_start_page():
    ...
    with ui.column().classes("w-full max-w-2xl mx-auto p-6 gap-6"):
        ...
        _build_header(db, state)

        ui.label("Buscar Registro").classes("rac-section-label")
        _build_search_section(db, state)

        ui.label("Novo Registro").classes("rac-section-label")

        for row_pair in [_TIPO_ORDER[:2], _TIPO_ORDER[2:]]:
            with ui.row().classes("w-full gap-3"):
                for tipo_key in row_pair:
                    _build_tipo_card(tipo_key, state)
        ...
        ).classes("w-full").props("size=lg unelevated no-caps")


def _build_header(db, state):
    with ui.card().classes("rac-card no-shadow w-full p-5"):
        with ui.row().classes("w-full items-center justify-between"):
            malote = state.get_active_malote()
            display = (
                _format_malote_date(malote) if malote
                else "Nenhum malote ativo"
            )
            ui.label(f"Malote: {display}").classes("text-subtitle1 text-weight-medium")
            ...
            with ui.row().classes("gap-2"):
                ui.button(
                    ...
                    icon="add",
                    color="primary",
                    on_click=lambda: _show_new_malote_dialog(db, state),
                ).props("dense flat no-caps")
                ui.button(
                    "Trocar",
                    icon="swap_horiz",
                    color="grey",
                    on_click=lambda: _show_malote_switcher(db, state),
                ).props("dense flat no-caps")


def _build_tipo_card(tipo_key: str, state):
    hex_color = TIPO_HEX[tipo_key]
    label = TIPO_LABELS[tipo_key]
    icon = TIPO_ICONS[tipo_key]

    card = ui.card().classes(
        "rac-tipo-card no-shadow flex-1"
    ).style(
    ...
    with card:
        ...
        ).style("padding: 16px 8px; min-height: 80px"):
            ui.icon(icon, size="lg").style(f"color: {hex_color}")
            ui.label(label).classes("text-subtitle2 text-weight-medium")

    card.on("click", lambda e, t=tipo_key: _on_tipo_click(t, state))
...
def _build_search_section(db, state):
    with ui.card().classes("rac-card no-shadow w-full p-4"):
        ...
        def on_search(e):
            val = e.args if isinstance(e.args, str) else search_select.value
            _perform_search(val, db, state)
...
def _show_new_malote_dialog(db, state):
    with ui.dialog() as dialog, ui.card().classes(
        "rac-card no-shadow p-6 min-w-[320px]"
    ):
        ui.label("Novo Malote").classes("text-h6 text-weight-bold")
        date_input = ui.input(
            "Data do malote",
        ...
        with ui.row().classes("w-full justify-end gap-2 mt-4"):
            ...
            ui.button(
                ...
                on_click=lambda: _create_malote(
                    date_input.value, db, state, dialog
                ),
...
def _create_malote(date_text, db, state, dialog):
    ...
    try:
        ...
        dialog.close()
        ui.notify(
            f"Malote criado: {_format_malote_date(malote)}", type="positive"
        )
        ui.navigate.to("/")
    except Exception as e:
        ErrorHandler.handle_error(
            e, context=ErrorContext.MALOTE, show_dialog=False
        )
        ui.notify(f"Erro: {e}", type="negative")


def _show_malote_switcher(db, state):
    ...
    with ui.dialog() as dialog, ui.card().classes(
        ...
        ui.label("Malotes").classes("text-h6 text-weight-bold")
            ...
            with ui.list().classes("w-full"):
                for m in malotes:
                    ...
                    with ui.item(on_click=lambda mal=m: _select_malote(
                        mal, state, dialog
                    )).classes("cursor-pointer rounded-xl"):
                        with ui.item_section().classes("w-full"):
                            with ui.row().classes("items-center w-full gap-2"):
                                ...
                                ui.label(display).classes(
                                    "text-body1"
                                    + (" text-weight-bold" if is_active else "")
                                )
        ...
        with ui.row().classes("w-full justify-end gap-2 mt-2"):
            ui.button(
                ...
                on_click=lambda: [
                    dialog.close(),
                    _show_new_malote_dialog(db, state),
                ],
            ).props("dense flat no-caps")
            ui.button(
                "Fechar", color="grey", on_click=dialog.close
            ).props("flat no-caps")
