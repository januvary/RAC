...
def full_setup(db, malote, paciente, catalog_items):
    ...
    db.set_registro_items(reg.id, [(i.id, 1, "", 0) for i in items])
    return {
        "db": db,
        "malote": malote,
        "paciente": paciente,
...
class TestRegistroItems:
    def test_set_and_get_items(self, db, malote, paciente, catalog_items):
        ...
        db.set_registro_items(r.id, [(iid, 1, "", 0) for iid in item_ids])
    ...
    def test_set_items_replaces(self, db, malote, paciente, catalog_items):
        r = db.create_registro("entrada", paciente.id, malote.id)
        db.set_registro_items(r.id, [(catalog_items[0].id, 1, "", 0)])
        db.set_registro_items(
            r.id, [(catalog_items[1].id, 1, "", 0), (catalog_items[2].id, 1, "", 0)]
        )
        items = db.get_items_by_registro(r.id)
        assert len(items) == 2
    ...
    def test_get_items_by_paciente_distinct(self, full_setup):
        ...
        r2 = db.create_registro("renovacao", paciente.id, malote.id)
        db.set_registro_items(r2.id, [(items[0].id, 1, "", 0), (items[2].id, 1, "", 0)])
...
class TestExportHelpers:
    ...
    def test_export_multiple_registros(self, full_setup):
        ...
        db.set_registro_items(r2.id, [(items[0].id, 1, "", 0)])
